  ...
  return <div className="mx-auto max-w-[1180px] px-4 pb-20 pt-7 sm:px-6 lg:px-8">
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-[.9fr_1.1fr]">
      <div className="space-y-5">
        <SettingsSection title="Workspace identity" code="ENVIRONMENT"><Setting label="Signed-in account" value={email} mono /><Setting label="Environment" value={hosted ? "hosted" : "revive-local"} /><Setting label="State backend" value={hosted ? "Postgres" : ".revive/console-state.json"} mono /></SettingsSection>
        <SettingsSection title="Integration readiness" code="SYSTEMS"><Readiness label="Microsoft Entra PKCE" ready={entra} /><Readiness label="Postgres job queue" ready={hosted} /><Readiness label="Signed delivery webhook" ready={delivery} /><Readiness label="Nango credential vault" ready={Boolean(process.env.NANGO_SECRET_KEY)} /><Readiness label="Auth0 Token Vault" ready={Boolean(process.env.AUTH0_DOMAIN)} /></SettingsSection>
      </div>
      <div className="space-y-5">
        <SettingsSection title="Recovery policy" code="POLICY"><Policy label="Recovery link lifetime" value="15 minutes" detail="Expired capabilities cannot resume a run." /><Policy label="Unknown provider errors" value="Escalate" detail="Never silently retry an unrecognized credential failure." /><Policy label="Credential delivery" value="Opaque lease" detail="Workers receive a connection reference and fencing generation." /><Policy label="Side-effect replay" value="Reconcile first" detail="Mutating actions reuse a stable idempotency key." /></SettingsSection>
        <section className="recorder-panel overflow-hidden border border-[#151922] text-[#151922]"><div className="border-b border-[#151922] px-5 py-4"><div className="font-mono text-[8px] tracking-[.11em] text-[#2e49c8]">SIDECAR QUICKSTART</div><h2 className="mt-2 text-[13px] font-semibold">Process-independent recovery</h2></div><pre className="overflow-x-auto bg-[#fbfcf8] p-5 font-mono text-[9.5px] leading-5 text-[#4f5866]"><code>{`from revive import Engine, Provider, PostgresCheckpointStore\n\nstore = PostgresCheckpointStore(DATABASE_URL)\nprovider = Provider("microsoft", TOKEN_URL, scopes=SCOPES)\nengine = Engine(provider, store, base_url=RECOVERY_URL)\n\nresult = engine.run(run_id, steps, credential_lease, SCOPES)`}</code></pre></section>
      </div>
    </div>
  </div>;
}
