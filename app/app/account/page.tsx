...
export default async function AccountPage() {
  ...
  return <div className="mx-auto max-w-[980px] px-4 pb-20 pt-7 sm:px-6 lg:px-8"><PageHeader eyebrow="Account" title="Profile" description="The identity currently authorized to access this local Revive console." actions={<StatusBadge tone="neutral">local authentication</StatusBadge>} /><section className="instrument-panel mt-5 overflow-hidden"><SectionHeading title="Account identity" /><Row label="Email" value={session.email} mono /><Row label="Session" value="HMAC-signed, 7-day expiry" /><Row label="Authentication mode" value="Local sandbox" /></section><div className="mt-5 border border-[#ead9ba] bg-[#fff7e8] p-4 text-[10.5px] leading-5 text-[#7a551e]">Hosted SSO, MFA, SCIM and role assignment are not implemented. Do not treat local console authentication as enterprise RBAC.</div></div>;
}
