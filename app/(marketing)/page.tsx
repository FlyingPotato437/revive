...
const actionTypes = [
  ...
  ["Resume · verified", "Check staleness, then resume or replan the exact checkpoint with a signed callback, generation fencing and replay safety."],
];
...
export default function Home() {
  return <div className="revive-home">
      ...
      <div className="grid lg:grid-cols-[minmax(0,1.04fr)_minmax(460px,.96fr)] lg:items-stretch">
        <div className="relative flex min-w-0 items-center px-5 py-16 sm:px-8 lg:px-[max(3.5rem,calc((100vw-1380px)/2+2rem))] lg:py-28">
          <div className="hero-intro-content max-w-[650px]">
            ...
            <div className="mt-9 flex flex-wrap gap-3"><Link href="/signup?next=%2Fapp%2Fquickstart" className="inline-flex h-12 items-center rounded-[8px] bg-[#4967f2] px-5 text-[13px] font-semibold text-white transition hover:bg-[#3954d9] active:translate-y-px">Install free detector</Link><Link href="/signup?next=%2Fapp%2Fdetector" className="inline-flex h-12 items-center rounded-[8px] border border-[#d9ddd6] px-5 text-[13px] font-semibold text-[#151922] transition hover:border-[#151922] hover:bg-white active:translate-y-px">Paste a dead trace</Link></div>
            <p className="mt-6 font-mono text-[9px] leading-5 text-[#7b8491]">DETECT FREE · RESOLVE ON FIXED PLANS · RESUME OR REPLAN SAFELY</p>
          </div>
        </div>
        <HeroVisual />
      </div>
    ...
    <section aria-label="Recovery evidence" className="border-b border-[#e0e3dd] bg-[#fbfcf8]"><div className="mx-auto flex max-w-[1380px] flex-col gap-4 px-5 py-6 sm:px-8 md:flex-row md:items-center md:justify-between"><p className="max-w-[850px] text-[11.5px] leading-5 text-[#596273]"><span className="font-semibold text-[#151922]">The recovery path is tested, not implied.</span> Revive verifies worker replacement, duplicate delivery, stale credentials, generation fencing, reconciliation and signed same-run continuation.</p><Link href="/benchmarks" className="shrink-0 text-[10.5px] font-semibold text-[#2e49c8] hover:underline">Inspect evidence and limits →</Link></div></section>

    <section id="product" className="mx-auto grid max-w-[1380px] gap-16 px-5 py-24 sm:px-8 lg:grid-cols-[.74fr_1.26fr] lg:py-36">
      <Reveal className="lg:sticky lg:top-28 lg:self-start"><p className="text-[11px] font-semibold text-[#4967f2]">One recovery loop</p><h2 className="mt-5 max-w-[500px] text-[clamp(36px,4.5vw,56px)] font-semibold leading-[1.01] tracking-[-.04em] text-[#151922]">Measure dead runs first. Recover only the ones a person can save.</h2><p className="mt-6 max-w-[440px] text-[14px] leading-6 text-[#687180]">Detector gives builders useful evidence on day one. Secure resolution turns measured loss into recovered work.</p></Reveal>
