    ...
    <main className="min-h-[100dvh] bg-[#f4f5f1] text-[#151922]">
      ...
      <div className="flex flex-col items-center px-4 py-14 sm:py-20">
        <SignUp
          path="/sign-up"
          routing="path"
          signInUrl={`/sso?next=${encodeURIComponent(next)}`}
          forceRedirectUrl={bridgeUrl}
          fallbackRedirectUrl={bridgeUrl}
          appearance={clerkAppearance}
        />
        <div className="mt-6 text-center text-[13px] text-[#66707e]">
          <Link href={`/signup?method=password&next=${encodeURIComponent(next)}`} className="font-semibold text-[#151922] underline decoration-[#c7ccd2] underline-offset-4 transition hover:decoration-[#151922]">
