...
export function EventLog({ logs }: { logs: LogLine[] }) {
  ...
  useEffect(() => bottomRef.current?.scrollIntoView({ behavior: "smooth", block: "end" }), [shown.length]);

  return (
    <section className="flex h-full flex-col overflow-hidden border border-[#242936] bg-[#131720] shadow-lift">
      <header className="flex h-11 items-center justify-between border-b border-white/10 px-4">
        <div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#42d392]" /><span className="text-[11px] font-medium text-white/80">Recovery event stream</span><span className="font-mono text-[9px] text-white/30">live</span></div>
