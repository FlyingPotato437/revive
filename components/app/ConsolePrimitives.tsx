...
export function SummaryStrip({ items }: { items: Array<{ label: string; value: string; detail?: string; tone?: "ok" | "warn" | "fail" | "cobalt" }> }) {
  return <section className="instrument-panel grid overflow-hidden border-[#151922] sm:grid-cols-2 lg:grid-cols-4">{items.map((item, index) => <div key={item.label} className={`relative min-w-0 px-4 py-4 sm:px-5 ${index ? "border-t border-[#cdd2d7] sm:border-l sm:border-t-0" : ""}`}><div className="font-mono text-[8px] tracking-[.1em] text-[#7b8491]">{item.label.toUpperCase()}</div><div className={`mt-2 truncate text-[19px] font-semibold tracking-[-.035em] ${item.tone === "ok" ? "text-[#18724e]" : item.tone === "warn" ? "text-[#9a5c15]" : item.tone === "fail" ? "text-[#af4039]" : item.tone === "cobalt" ? "text-[#2e49c8]" : "text-[#151922]"}`}>{item.value}</div>{item.detail && <div className="mt-1 truncate font-mono text-[8.5px] text-[#828b97]">{item.detail}</div>}</div>)}</section>;
}
